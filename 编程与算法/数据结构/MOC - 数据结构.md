...
计算机科学引论提供全学科视角，本课程把其中“数据如何组织、算法如何评价”两条主线向下展开。以下入口用于在概览与专业细节之间往返，而不是复制两门课的正文。

| 引论中的入口 | 数据结构课程中的深入内容 | 衔接重点 |
| --- | --- | --- |
| 数据结构概览 | [[1.02.02 数据结构|逻辑结构与存储结构]] | 从结构类型清单进入统一定义与表示方法 |
| 复杂度与规模效应 | [[1.04.03 算法的时间复杂度|时间复杂度]] · [[1.04.04 算法的空间复杂度|空间复杂度]] | 从数量级直觉进入渐近分析 |
| 编程与语言 | [[1.02.03 数据类型和抽象数据类型|数据类型与 ADT]] · [[1.03 抽象数据类型的表示与实现|ADT 的实现]] | 从编程流程进入接口、表示和操作 |
| 数据库 | [[7.03.05 B+ 树|B+ 树]] · [[MOC - 第7章 查找#7.4 散列表的查找|散列表]] | 从数据模型进入索引结构与查询代价 |
| 二级存储 | [[8.07.01 外部排序的基本方法|外部排序]] | 从介质性能进入以 I/O 次数为核心的算法设计 |
