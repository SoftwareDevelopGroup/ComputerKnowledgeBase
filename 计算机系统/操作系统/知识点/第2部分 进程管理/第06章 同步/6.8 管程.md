...
2. **访问控制问题**：管程内部的 `mutex` 锁只能锁定管程内部的函数调用，但**无法阻止外来代码直接读取或修改共享变量**。真正的解决之道在于引入操作系统底层的**“附加机制”**（对应本书[[第14章 系统保护]]），例如利用硬件或操作系统提供的内存隔离、权限控制。

> [!info] 章节导航
> 上一节：[[6.7 经典同步问题]]　｜　章节：[[第06章 同步]]　｜　下一节：[[6.9 同步例子]]
