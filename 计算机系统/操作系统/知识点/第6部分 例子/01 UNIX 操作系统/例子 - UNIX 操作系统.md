...
3. 调度器在 [[第05章 进程调度]] 的策略下为可运行线程分配 CPU。
4. 程序经系统调用请求文件、内存、网络或进程服务；发生时从用户态切换到内核态。
5. 正常退出、信号、异常或父进程等待都会改变进程状态并触发资源回收。

> [!important] 进程、程序与线程
> **程序**是静态文件；**进程**是正在执行的资源容器；**线程**是实际调度执行单元。它们的区别与 [[第03章 进程]]、[[第04章 多线程编程]] 共同构成 UNIX 进程模型的理解前提。
...
| 读写数据 | 页缓存、逻辑块、文件偏移 | [[第11章 文件系统实现]] |
| 写回介质 | 缓冲、设备队列、驱动 | [[第13章 IO系统]] |
...
编辑器和编译器看到的是各自独立的虚拟地址空间。代码段、堆、栈和映射文件按页组织；缺页时内核从后备存储取回页面或分配零页。内存不足时，页面可能被换出到交换空间，具体机制见 [[第09章 虚拟内存管理]] 与 [[12.6 交换空间管理]]。
...
若两个进程同时修改同一个文件，文件系统的写入成功不等于应用语义正确。临界区、互斥、死锁与文件锁分别见 [[第06章 同步]]、[[第07章 死锁]] 和 [[10.5 文件共享]]。
...
设备延迟、排队与文件布局会影响用户可感知的响应时间。机械磁盘可使用调度降低寻道，SSD 仍有队列、垃圾回收和写放大等不同瓶颈；详见 [[第12章 大容量存储设备]]。
...
网络服务进一步引入远程身份、协议、缓存和故障问题。远程文件系统可被挂载到本地命名空间，但其一致性和认证不应默认等同于本地磁盘。保护与安全的概念边界见 [[第14章 系统保护]]、[[第15章 系统安全]]。
...
- 相关入口：[[第02章 操作系统结构]]、[[第10章 文件系统]]、[[第11章 文件系统实现]]、[[第13章 IO系统]]、[[第14章 系统保护]]、[[第15章 系统安全]]。
